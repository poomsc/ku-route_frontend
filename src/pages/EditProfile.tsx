    ...
    <div className="blue-bg py-5">
      ...
      >
        ...
        <InputGroup className="rounded-10 bg-white shadow mb-4">
          ...
          >
            {(title ? title?.length : userInfo?.DisplayName ? userInfo?.DisplayName.length : 0)}/50
          </div>
        </InputGroup>
        ...
        >
          <SMTDropdown
            ...
            value={userFaculty ? facultyList[newSelected]?.value : facultyList[currentFaculty]?.value}
        ...
        <InputGroup className="rounded-10 bg-white shadow mb-4">
          ...
          >
            {about ? about?.length : userInfo?.About ? userInfo?.About.length : 0}/1000
          </div>
        </InputGroup>
        
        <div className="d-flex justify-content-end">
          <div className="mx-2"></div>
          <Button disabled={!saveButtonClickable}
                  style={{ width: '7rem' }}
                  type="submit"
                  onClick={(e) => {saveCurrentState(title, about, userFaculty)}}
          >
            บันทึก
          </Button>
        </div>
